...
imageContainerdiv = document.getElementById("imageContainer");
...
function initial(){
    for(var i=0; i<  galleryImagesSrc.length; i++){
        ...
        imageContainerdiv.appendChild(galleryImages[i]);
	...
	for(i=0; i<filesSelected.length; i++){
		...
		fileReader.onload = function(fileLoadedEvent) 
		{
			...
			//   alert("clicked");
                    ...
                    imageContainerdiv.appendChild(galleryImages[k]);
...
function loadImageFileAsURL()
{
    ...
    if (filesSelected.length > 0)
    {
		...
		galleryImages[len].src = fileLoadedEvent.target.result;
                imageContainerdiv.appendChild(galleryImages[len]);
...
function undoImage(){
	...
	imageContainerdiv.appendChild(galleryImages[i]);
	galleryImagesSrc.push( galleryImages[i].getAttribute('src'));
    }
}
